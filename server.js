...
const THEMES = {
  ...
  NEE:  { theme: "Clean Energy / Utilities", benchmark: "XLU" },
  ANET: { theme: "AI / Networking Infrastructure", benchmark: "SMH" },
...
app.get("/api/analyze", async (req, res) => {
  const tickers = req.query.tickers
    ? req.query.tickers.split(",").map(t => t.trim().toUpperCase()).filter(Boolean).slice(0, 50)
    : WATCHLIST;
